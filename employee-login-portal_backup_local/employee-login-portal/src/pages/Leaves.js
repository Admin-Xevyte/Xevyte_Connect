...
function Leaves() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      setFile(file); // <-- keeps the file ready for re-submit
    }
  } catch (err) {
  ...
  if (startDate && endDate) {
    ...
    if (
      ...
      (
        ...
        startDate.getDay() === 0 ||
        startDate.getDay() === 6 ||
        holidayDateStrings.has(startDate.toISOString().split('T')[0])
      )
    ) {
    ...
    if (
      ...
      type === 'Sick' &&
      (+leaveBalance.sickUsed + count) > +leaveBalance.sickTotal
    ) {
      setShowLOPAlert(true);
    } else {
      setShowLOPAlert(false);
...
const handleSaveDraft = async () => {
    ...
    const dto = {
        ...
        startDate: formatDate(leaveRequest.startDate), // ✅ Use the helper function
        endDate: formatDate(leaveRequest.endDate),     // ✅ Use the helper function
...
const handleSubmitLeave = async (e) => {
    ...
    if (leaveRequest.type === "Sick" && totalDays > 2 && !file && !leaveRequest.fileName) {
    ...
    try {
        ...
        if (leaveRequest.id) {
            ...
            const draftDto = {
                ...
                type: leaveRequest.type,
                // ✅ Use helper for proper date format
                startDate: leaveRequest.startDate instanceof Date
                    ? formatDateToBackend(leaveRequest.startDate)
                    : leaveRequest.startDate,
                endDate: leaveRequest.endDate instanceof Date
                    ? formatDateToBackend(leaveRequest.endDate)
            ...
            // Case 2: Submitting a new leave request
            const formData = new FormData();
            const dto = {
                employeeId: employeeId,
                type: leaveRequest.type,
      ...
      <div className="main-content">
        ...
        <div style={{ flex: '1', padding: '20px', overflowY: 'auto' }}>
          ...
          <div style={{ marginBottom: '20px', display: 'flex', gap: '15px' }}>
            ...
            <button onClick={handleMyTasksClick} style={{ padding: '10px 20px', fontSize: '16px', backgroundColor:  "#1890ff", color: 'white', border: 'none', borderRadius: '5px',  boxShadow: '0 2px 5px rgba(0,0,0,0.15)' }}>My Tasks</button>
          </div>
