    ...
    <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", backgroundColor: "rgba(0, 0, 0, 0.5)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 1000 }}>
      <div style={{ padding: "15px", border: "1px solid #ddd", borderRadius: "8px", width: "600px", backgroundColor: "#fff", boxShadow: "0 4px 6px rgba(0, 0, 0, 0.1)", position: "relative" }}>
        ...
        <form onSubmit={handleSubmit}>
          {/* Client */}
          <label style={labelStyle} htmlFor="client-select">Client <span style={{ color: "red" }}>*</span></label>
          <select id="client-select" value={client} onChange={(e) => { setClient(e.target.value); if (e.target.value) setErrors((prev) => ({ ...prev, client: "" })); }} style={{ padding: "8px", margin: "5px", border: "1px solid #ccc", borderRadius: "5px", width: "99%" }}>
            <option value="">-- Select Client --</option>
            {clients.map((c, idx) => (<option key={idx} value={c}>{c}</option>))}
          </select>
          {errors.client && (<div style={{ color: "red", marginLeft: "5px" }}>{errors.client}</div>)}

          {/* Project */}
          <label style={labelStyle} htmlFor="project-select">Project <span style={{ color: "red" }}>*</span></label>
          <select id="project-select" value={project} onChange={(e) => { setProject(e.target.value); if (e.target.value) setErrors((prev) => ({ ...prev, project: "" })); }} style={{ padding: "8px", margin: "5px", border: "1px solid #ccc", borderRadius: "5px", width: "99%" }}>
            <option value="">-- Select Project --</option>
            {projects.map((p, idx) => (<option key={idx} value={p}>{p}</option>))}
          </select>
          {errors.project && (<div style={{ color: "red", marginLeft: "5px" }}>{errors.project}</div>)}
