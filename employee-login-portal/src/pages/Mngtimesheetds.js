  ...
  const confirmFreeze = async () => {
    ...
    for (const emp of filteredSubordinates) {
      try {
        ...
        const res = await fetch('http://13.234.30.186:8080/daily-entry/freeze', {
          method: 'PUT',
          headers: {
                  ...
                  <tbody>
                    {filteredSubordinates.map((emp) => (
                      <tr key={emp.id || emp.employeeId}>
                        <td>
                          <button type="button" style={{ color: 'blue', background: 'none', border: 'none', cursor: 'pointer', padding: 0, font: 'inherit' }} onClick={() => handleEmployeeClick(emp.employeeId)}>{emp.employeeId}</button>
                        </td>
                        <td>{emp.name}</td>
                        <td>{emp.email}</td>
                      </tr>
